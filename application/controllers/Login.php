<?php

if (!defined('BASEPATH'))
    exit('No direct script access allowed');

class Login extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
    }

    public function index()
    {
		if($this->session->userdata('logined') && $this->session->userdata('logined') == true)
		{
			redirect('home');
		}
		
		if(!$this->input->post())
		{
			$this->load->view('login');
		}
		else
		{
			if(
				strtolower($this->input->post('username')) == LOGIN_USERNAME &&
				$this->input->post('password')  == LOGIN_PASSWORD
			)
			{
				$this->session->set_userdata('logined', true);
				
				redirect("home");
			}
			else 
			{
                $user=$this->input->post('username');
                $pass=$this->input->post('password');

                $cek=$this->db->query('select * from user where username="'.$user.'" and password="'.$pass.'"')->row();
                // print_r($_POST);
                
                if ($cek) {
                    $this->session->set_userdata('logined', true);
                    $this->session->set_userdata('data', $cek);
                    redirect("http://rgde.riset.pcr.ac.id/hackaton/home/dashboard_pd");
                }
               
				redirect("/");
			}
		}
        
    } 
	
	public function logout()
    {
		$this->session->unset_userdata('logined');
		redirect("/");
    } 
}

/* End of file Workflows.php */
/* Location: ./application/controllers/Workflows.php */
/* Please DO NOT modify this information : */
/* Generated by Harviacode Codeigniter CRUD Generator 2017-04-15 00:43:10 */
/* http://harviacode.com */